import random
import os
from telegram import Update
from telegram.ext import ApplicationBuilder, CommandHandler, ContextTypes

# ====== НАСТРОЙКИ ======
import os
from telegram import Update
from telegram.ext import ApplicationBuilder, CommandHandler
import random

TOKEN = os.getenv("TOKEN")
if not TOKEN:
    raise ValueError("Токен не найден! Проверь Environment Variables на Railway")
PHOTOS_DIR = "photos"

# ====== ПУЛ ФРАЗ (50) ======
phrases = [
    "Ты в безопасности",
    "С тобой всё в порядке",
    "Это чувство пройдёт",
    "Можно выдохнуть",
    "Ты не обязана решать всё сейчас",
    "Ты справляешься, даже если так не кажется",
    "Можно замедлиться",
    "Ты не одна",
    "Твои чувства важны",
    "Не нужно быть сильной прямо сейчас",

    "Всё, что ты чувствуешь, — нормально",
    "Ты имеешь право на паузу",
    "Можно просто пережить этот момент",
    "Ты не сломана",
    "Это временно",
    "Ты уже делаешь достаточно",
    "Не нужно торопиться",
    "Ты не обязана всё контролировать",
    "Можно позволить себе отдых",
    "Ты важна",

    "Даже сейчас ты ценная",
    "Ты не обязана быть идеальной",
    "Можно дышать медленно",
    "Этот день не определяет тебя",
    "Ты больше, чем эта тревога",
    "Леночка, ты справишься",
    "Ты не обязана понимать всё сразу",
    "Можно быть мягкой к себе",
    "Ты не перегибаешь",
    "Твоя чувствительность - дар",

    "Твои эмоции не делают тебя слабой",
    "Можно переживать и всё равно быть в порядке",
    "Ты не обязана сейчас что-то доказывать",
    "Ты не плохая за то, что тебе тяжело",
    "Это не навсегда",
    "Ты заслуживаешь спокойствия",
    "Ты заслуживаешь заботы",
    "Леночка, можно просто быть собой",
    "Ты не обязана справляться одна",
    "Всё, что сейчас нужно — немного времени",

    "Ты не обязана быть продуктивной",
    "Ты имеешь право на слабость",
    "Даже если внутри шторм - это пройдёт",
    "Ты не делаешь ничего неправильного",
    "Не торопись, всё решим шаг за шагом",
    "Ты справлялась раньше - справишься и сейчас",
    "Ты сильнее, чем кажешься",
    "Можно быть добрее к себе",
    "Ты заслуживаешь тепла",
    "Ты важна, даже когда тебе тяжело"
]

# ====== КОМАНДЫ ======

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Привет 🤍\n"
        "Я буду иногда присылать тёплые слова и фотографии.\n\n"
        "Команды:\n"
        "/warm — тёплая фраза\n"
        "/dog — фото таксы\n"
        "/random — случайно"
    )

async def warm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = random.choice(phrases)
    await update.message.reply_text(text)

async def dog(update: Update, context: ContextTypes.DEFAULT_TYPE):
    photos = os.listdir(PHOTOS_DIR)
    photo_name = random.choice(photos)
    photo_path = os.path.join(PHOTOS_DIR, photo_name)

    with open(photo_path, "rb") as photo:
        await update.message.reply_photo(photo)

async def random_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # 50% шанс — текст, 50% — фото
    if random.choice([True, False]):
        await warm(update, context)
    else:
        await dog(update, context)

# ====== ЗАПУСК ======

app = ApplicationBuilder().token(TOKEN).build()

app.add_handler(CommandHandler("start", start))
app.add_handler(CommandHandler("warm", warm))
app.add_handler(CommandHandler("dog", dog))
app.add_handler(CommandHandler("random", random_all))

app.run_polling()


